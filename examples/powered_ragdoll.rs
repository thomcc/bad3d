...
fn main() -> Result<()> {
    ...
    let mut window = DemoWindow::new(DemoOptions {
        title: "Powered ragdoll physics test",
        view: M4x4::look_at(vec3(0.0, -8.0, 0.0),
                            vec3(0.0, 0.0, 0.0),
                            vec3(0.0, 0.0, 1.0)),
    ...
    for size in body_sizes.iter() {
        demo_objects.push(DemoObject::new_box(&window.display, *size, V3::zero(), None)?);
    ...
    demo_objects.push(DemoObject::new_box(&window.display,
        vec3(2.0, 0.1, 0.1), vec3(0.0, 0.0, -0.5), None)?);
    demo_objects.push(DemoObject::new_box(&window.display,
        vec3(2.0, 0.4, 0.1), vec3(0.0, 1.0, -0.5), None)?);
    ...
    while window.is_up() {
        ...
        let need_reset = bodies[0..body_sizes.len()].iter().find(|b|
            b.borrow().pose.position.length() > 25.0).is_some();
        if need_reset {
